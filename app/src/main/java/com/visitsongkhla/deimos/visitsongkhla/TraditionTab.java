...
public class TraditionTab extends Fragment {
    ...
    private FirebaseRecyclerAdapter<CommonModel, TraditionTab.NewsViewHolder> RVAdapter;
    DatabaseReference mRef;
    public static int positionIndex = -1;
    public static int topView = -1;
    LinearLayoutManager linearLayoutManager ;
    private TraditionTab mActivity;
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        new TraditionTab.MyTasks(TraditionTab.this).execute((Void) null);
...
/*
    ...
    class MyTasks extends AsyncTask<Void, Void, Void> {
        ListView theList;
        public MyTasks(TraditionTab activity) {
            mActivity = activity;
        }
                ...
                public void run() {
                    ...
                    handler.postDelayed(new Runnable() {
                        ...
                        public void run() {
                            ...
                            FirebaseRecyclerOptions foodOptions = new FirebaseRecyclerOptions.Builder<CommonModel>().setQuery(Q,CommonModel.class).build();
                            RVAdapter = new FirebaseRecyclerAdapter<CommonModel, TraditionTab.NewsViewHolder>(foodOptions) {
                                @Override
                                protected void onBindViewHolder(@NonNull TraditionTab.NewsViewHolder holder, int position, final CommonModel model) {
                                    holder.setTitle(model.getTitle());
                                    // holder.setDes(model.getDescription());
                                    holder.setImage(getContext(), model.getUrl());
                                    holder.mView.setOnClickListener(new View.OnClickListener() {
                                        ...
                                        public void onClick(View v) {
                                            final String url = model.getUrl();
                                            Intent intent = new Intent(getContext(), Gallery_Event.class);
                                            intent.putExtra("image_name", model.getTitle());
                                            intent.putExtra("image_url", model.getUrl());
                                            intent.putExtra("Des", model.getDes());
                                            intent.putExtra("Local",model.getLocation());
                                ...
                                @NonNull
                                @Override
                                public TraditionTab.NewsViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
                                    View view = LayoutInflater.from(parent.getContext())
                                            .inflate(R.layout.listitem_places, parent, false);

                                    return new TraditionTab.NewsViewHolder(view);
                                }
                            };
                            mRecyclerView.setAdapter(RVAdapter);
                            RVAdapter.startListening();
