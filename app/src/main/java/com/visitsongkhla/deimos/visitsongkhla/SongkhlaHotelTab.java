...
public class  SongkhlaHotelTab extends Fragment {
    ...
    private FirebaseRecyclerAdapter<CommonModel, SongkhlaHotelTab.NewsViewHolder> RVAdapter;
    DatabaseReference mRef;
    public static int positionIndex = -1;
    public static int topView = -1;
    LinearLayoutManager linearLayoutManager ;
    private SongkhlaHotelTab mActivity;
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        new SongkhlaHotelTab.MyTasks(SongkhlaHotelTab.this).execute((Void) null);
...
/*
    ...
    class MyTasks extends AsyncTask<Void, Void, Void> {
        ListView theList;
        public MyTasks(SongkhlaHotelTab activity) {
            mActivity = activity;
        }
        ...
        protected Void doInBackground(Void... params) {
            ...
            /*FirebaseRecyclerOptions foodOptions = new FirebaseRecyclerOptions.Builder<CommonModel>().setQuery(Q,CommonModel.class).build();
            RVAdapter = new FirebaseRecyclerAdapter<CommonModel, SongkhlaHotelTab.NewsViewHolder>(foodOptions) {
                @Override
                protected void onBindViewHolder(@NonNull SongkhlaHotelTab.NewsViewHolder holder, int position, final CommonModel model) {
                    holder.setTitle(model.getTitle());
                    // holder.setDes(model.getDescription());
                    holder.setImage(getContext(), model.getUrl());
                    holder.mView.setOnClickListener(new View.OnClickListener() {
                ...
                @NonNull
                @Override
                public SongkhlaHotelTab.NewsViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
                    View view = LayoutInflater.from(parent.getContext())
                            .inflate(R.layout.listitem_places, parent, false);

                    return new SongkhlaHotelTab.NewsViewHolder(view);
                }
            };
            mRecyclerView.setAdapter(RVAdapter);
            RVAdapter.startListening();
